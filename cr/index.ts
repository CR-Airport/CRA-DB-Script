...
const FILE_PHRASES = "phrase.list.json";

export async function cr(schemaPath: string, dataDir: string): Promise<string> {
    ...
    await populate(
        ...
        async function(this: Database): Promise<void> {
            await crDb.populate(
                db,
                await readJsonData<Array<Phrase>>(Path.join(dataDir, FILE_PHRASES)),
            );
        }
    );
